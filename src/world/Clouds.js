 ...
 *
 * A translucent sphere slightly above the surface whose alpha comes from
 * 4-octave simplex fBm evaluated on the (slowly rotating) shell direction.
 * Clouds are lit by the planet's sun direction with a soft wrap term, fade
 * at the silhouette so the shell edge never reads as a hard ring, and are
 * double-sided so they read correctly from under the deck.
 */

// Compact 3D simplex noise (Ashima Arts / Ian McEwan, public domain).
...
export class Clouds {
   ...
   * @param {THREE.Vector3} sunDirUniformTarget shared live planet→sun vector
   */
  constructor(descriptor, sunDirUniformTarget) {
    const radius = descriptor.radius * 1.035 + descriptor.relief;
    this.speed = descriptor.clouds.speed;
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uColor: { value: descriptor.clouds.color },
        uCoverage: { value: descriptor.clouds.coverage },
        uTime: { value: 0 },
      },
      transparent: true,
      depthWrite: false,
  ...
  update(dt, elapsed) {
    ...
    // Physical drift: the whole deck slowly rotates around the planet.
    this.mesh.rotation.y += this.speed * 0.6 * dt;
  }
}
